    ...
    E: Engine,
{
    ...
    info!("aff_size:{:?}, exp_size:{:?}, proj_size:{:?}", aff_size, exp_size, proj_size);
    ...
    E: Engine,
{
    pub fn create(d: opencl::Device, priority: bool) -> GPUResult<SingleMultiexpKernel<E>> {
        ...
        let n = std::cmp::min(max_n, best_n)*3;
        info!("max_n:{:?},best_n:{:?}", max_n, best_n);
        ...
        G: CurveAffine,
    {
        ...
        //info!("!!!lzy!!!window_size:{},num_window:{},num_groups", window_size, num_windows, num_groups);
        ...
        info!("!!!lzy!!!n:{},bucket_len:{}", n, bucket_len);
...
//const SOURCE_BIN: &[u8] = b"./multiexp/multiexp32.fatbin\0";
const SOURCE_BIN: &[u8] = b"./mul.ptx\0";
    ...
    E: Engine,
{
    pub fn create(ctx: CudaUnownedCtx, priority: bool) -> GPUResult<SingleMultiexpKernelCuda<E>> {
        ...
        info!("exp_bits:{:?}, mem:{:?}", exp_bits, mem);
        info!("max_n:{:?},best_n:{:?}", max_n, best_n);
        rustacuda::context::CurrentContext::set_current(&ctx.context)?;
        info!("{:?}",src);
         let module_data = CString::new(include_str!("./multiexp/multiexp32.ptx")).unwrap();
         let module1 = rustacuda::module::Module::load_from_string(&module_data)?;
        //let module1 = rustacuda::module::Module::load_from_file(src)?;
        info!("module1:{:?}", module1);
        ...
        G: CurveAffine,
    {
        ...
        info!("!!!lzy!!!window_size:{},num_window:{},num_groups:{},bucket_len:{}", window_size, num_windows, num_groups, bucket_len);
        // Each group will have `num_windows` threads and as there are `num_groups` groups, there will
        // be `num_groups` * `num_windows` threads in total.
        // Each thread will use `num_groups` * `num_windows` * `bucket_len` buckets.
        ...
        let result_bytes =
            4 * self.core_count * std::mem::size_of::<<G as CurveAffine>::Projective>();
        ...
        let mut results = vec![<G as CurveAffine>::Projective::zero(); 4 * self.core_count];
        unsafe {
    ...
    E: Engine,
{
        ...
        <G as groupy::CurveAffine>::Engine: crate::bls::Engine,
    {
        ...
        info!("!!!lzy!!!n:{:?},skip:{:?},chunk_size:{:?}", n, skip, chunk_size);
            ...
            let results = if n > 0 {
                bases
                    ...
                    .map(|((bases, exps), kern)| -> Result<<G as CurveAffine>::Projective, GPUError> {
                        //info!("11111");
                        let mut acc = <G as CurveAffine>::Projective::zero();
                        let mut kernn = kern.n;
                        if TypeId::of::<G>() == TypeId::of::<E::G2Affine>(){
                            kernn = kern.n/4;
                        }
                        for (bases, exps) in bases.chunks(kernn).zip(exps.chunks(kernn)) {
                            info!("bases.len:{:?},kern.n:{:?},kernn:{:?}", bases.len(), kern.n, kernn);
