...
#export NEPTUNE_DEFAULT_GPU=161
# checking gpu
gpu=""
type nvidia-smi
if [ $? -eq 0 ]; then
...
#################
#
#RUST_LOG=info RUST_BACKTRACE=1 ./lotus-bench p-run --storage-dir=/data/cache/.lotus-bench --sector-size=$size --max-tasks=2 --taskset=false --parallel-addpiece=2 --parallel-precommit1=2 --parallel-precommit2=2 --parallel-commit1=0 --parallel-commit2=0 &
